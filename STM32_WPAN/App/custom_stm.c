...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomCm200Hdle;                    /**< CM200_SensorDevKit handle */
  uint16_t  CustomWriteHdle;                  /**< CM200_UART_WRITE handle */
  uint16_t  CustomReadHdle;                  /**< CM200_UART_READ handle */
 ...
 */
#define COPY_CM200_SENSORDEVKIT_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x60,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_CM200_UART_WRITE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x61,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_CM200_UART_READ_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x62,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for CM200_SensorDevKit +
   *                                2 for CM200_UART_WRITE +
   *                                2 for CM200_UART_READ +
   *                                1 for CM200_UART_READ configuration descriptor +
   *                              = 6
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomCm200Hdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: CM200, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: CM200 \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomCm200Hdle,
                          ...
                          &(CustomContext.CustomWriteHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : WRITE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : WRITE \n\r");
  }
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : READ, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : READ \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_WRITE:
      ret = aci_gatt_update_char_value(CustomContext.CustomCm200Hdle,
                                       CustomContext.CustomWriteHdle,
                                       0, /* charValOffset */
                                       SizeWrite, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value WRITE command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value WRITE command\n\r");
      }
    ...
    case CUSTOM_STM_READ:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value READ command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value READ command\n\r");
      }
